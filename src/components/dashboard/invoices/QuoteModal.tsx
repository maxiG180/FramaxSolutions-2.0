...
export function QuoteModal({ isOpen, onClose, onQuoteSaved, editingQuoteId }: QuoteModalProps) {
    ...
    // Quote number state - editable by user
    ...
    // Load next quote number when creating new quote
    useEffect(() => {
        const loadNextQuoteNumber = async () => {
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error('Failed to load next quote number');
                }
                ...
                setQuoteNumber(`ORC-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 1000)).padStart(3, '0')}`);
            }
        };

        if (!editingQuoteId && isOpen && !quoteNumber) {
            loadNextQuoteNumber();
        }
    }, [editingQuoteId, isOpen, quoteNumber]);
    ...
    const resetForm = () => {
        ...
        setQuoteNumber(""); // Reset quote number - will be auto-loaded by useEffect
        setNotes("");
        setItems([{ id: "1", description: "", quantity: 1, price: 0, serviceId: null }]);
        setSaveError("");
        setIsEditMode(false);
        setIsClientDetailsExpanded(true);
        setIsQuoteDetailsExpanded(true);
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            // Use the stable preview quote number
    ...
    const handleSaveOnly = async () => {
        ...
        try {
            ...
            const response = await fetch(url, {
                ...
                body: JSON.stringify({
                    quoteNumber, // Send custom quote number to API
                    clientName,
                    clientEmail,
                    clientPhone,
                    clientContact,
    ...
    return (
        <>
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                >
                                                    ...
                                                    <motion.div
                                                        animate={{ rotate: isQuoteDetailsExpanded ? 180 : 0 }}
                                                        transition={{ duration: 0.2 }}
                                                    >
                                                        ...
                                                        >
                                                            <div className="space-y-4">
                                                                ...
                                                                <div className="space-y-2">
                                                                    <label className="text-sm text-white/60">{t.quoteModal.quoteNumber}</label>
                                                                    ...
                                                                    <p className="text-xs text-white/40">Número sequencial gerado automaticamente. Pode editar se necessário.</p>
                                                                </div>
                                                            ...
                                                            <div className="space-y-2">
                                                                ...
                                                                <div className="grid grid-cols-2 gap-2">
                                                                    <button
                                                                        ...
                                                                        className={`py-2.5 px-4 rounded-lg text-sm font-medium transition-all ${clientLanguage === 'pt'
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        className={`py-2.5 px-4 rounded-lg text-sm font-medium transition-all ${clientLanguage === 'en'
                                                                                ? 'bg-blue-500 text-white'
                                                                                : 'bg-white/5 border border-white/10 text-white/60 hover:bg-white/10'
                                                                            }`}
                                                                    >
                                ...
                                <div className="w-full max-w-[210mm] min-h-[297mm] bg-white text-black p-12 shadow-2xl relative flex flex-col">
                                    ...
                                    <div className="flex justify-between items-start mb-12">
                                        ...
                                        <div className="text-right">
                                            ...
                                            <p className="text-gray-700 font-bold text-lg">{quoteNumber || '...'}</p>
                                        </div>
                                    </div>
                                        ...
                                        <div className="w-64 space-y-2">
                                            {(taxRate && taxRate > 0) ? (
                                                <>
                                                    ...
                                                    <div className="flex justify-between text-sm text-gray-500">
                                                        <span>{previewT.quoteModal.tax} ({(taxRate * 100).toFixed(0)}%)</span>
            ...
            <EmailPreviewModal
                ...
                quoteNumber={savedQuoteNumber || quoteNumber}
